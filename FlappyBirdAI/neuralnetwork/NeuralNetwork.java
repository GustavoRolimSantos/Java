...
public class NeuralNetwork {
	...
	public double fits[], inputs[], neurons[][];
	public int layers_amount, genomes_per_generation, current_genome, current_generation;
	public int neurons_amount[];
	public double synapses[][][][];
	
	private double random_mutation_probability, min_weight, max_weight;
	
	private NeuronRetrieve neuronRetrieve;
	...
	public NeuralNetwork(int neurons_amount[], int genomes_per_generation, double random_mutation_probability, double min_weight, double max_weight) {
		...
		neuronRetrieve = new NeuronRetrieve(this);

		// If the file exists, load it and make the crossover. Else, init randomly
		if (neuronRetrieve.fileExists()) {
			try {
				neuronRetrieve.loadFromFile();
	...
	public NeuronRetrieve getSaveLoad() {
		return neuronRetrieve;
	}
